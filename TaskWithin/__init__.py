...
def creating_session(subsession):
    ...
    for player in subsession.get_players():
        ...
        if round<= C.NUM_PROUNDS + C.NUM_RROUNDS:
            trial = int(round-1)
        else: 
            trial = int(round - C.NUM_RROUNDS - 1 )
        print(player.round_number, trial)
        player.sBetweenBtn = random.choice(['left','right'])
        db = p.dbTrials
        trialValues = db.iloc[trial].astype(object).to_dict()
        print(trialValues)
...
class InfoBetween(Page):
    ...
    def is_displayed(player):
        return player.round_number == C.NUM_PROUNDS + C.NUM_RROUNDS
...
page_sequence = [InfoBetween, SideButton, Decision, Confidence]